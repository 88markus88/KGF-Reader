...
class BatteryMonitor{
  ...
  void
    ...
    begin(int address, Stream &SerialDevice, int pinForRSE),
  ...
  int 
    getUptime(),
    getMeasuredValuesChecksum(),
    getMeasuredValuesTstChecksum(),
    getSetValuesChecksum(),
    getSetValuesTstChecksum(),
    getBatteryLifeLeft(),
    getTemperature(),
  ...
  String
    getRawMessage(),  
    getStringField(String message, int idx); // MP
    // getMeasuredValues(); // MP
  ...
  int               pinRSE;   // pin for RSE - enable in/out of RS475 adapter
  //Stream            &bm_serial;
};
